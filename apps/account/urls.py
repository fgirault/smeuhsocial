...
urlpatterns = patterns(
    "",
    url(r"^email/$", "account.views.email",
        name="acct_email"),
    ...
    url(r"^password_change/$",
        "account.views.password_change", name="acct_passwd"),
    url(r"^password_set/$", "account.views.password_set",
        name="acct_passwd_set"),
    url(r"^password_delete/$", "account.views.password_delete",
        name="acct_passwd_delete"),
    url(r"^password_delete/done/$", PasswordDeleteView.as_view(),
        name="acct_passwd_delete_done"),
    url(r"^timezone/$", "account.views.timezone_change",
        name="acct_timezone_change"),

    url(r"^language/$", "account.views.language_change",
    ...
    # password reset
    url(r"^password_reset/$", "account.views.password_reset",
        name="acct_passwd_reset"),
    url(r"^password_reset/done/$", "account.views.password_reset_done",
        name="acct_passwd_reset_done"),
    url(r"^password_reset_key/(?P<uidb36>[0-9A-Za-z]+)-(?P<key>.+)/$",
        "account.views.password_reset_from_key", name="acct_passwd_reset_key"),

    # ajax validation
    (r"^validate/$", "ajax_validation.views.validate",
     {"form_class": SignupForm}, "signup_form_validate"),
)
